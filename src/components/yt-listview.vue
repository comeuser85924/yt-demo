<template>
  <div class="yt-home">
    ...
    <div v-if="$store.state.loading" class="yt-load" >
        <span>載入中</span>
...
export default {
  ...
  data(){
    return{
      ...
      nextPage:null,
      apikey:process.env,
      localData:JSON.parse(localStorage.getItem('collect'))
    }
  ...
  beforeMount(){
    if(this.$store.state.youtubeUrl == null){
      this.$store.commit('SET_YOUTUBE_URL', 'https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&chart=mostPopular&maxResults=12&key=AIzaSyDiX_hFkzy9enM7e4eh1oWovmTb0a0r4Mc')
    }
  ...
  async mounted(){
    ...
    await axios.get(this.$store.state.youtubeUrl)
    .then((response) => {
